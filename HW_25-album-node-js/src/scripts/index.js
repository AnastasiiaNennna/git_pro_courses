...
const albumsResourse = new Http(ALBUMS_URL);
const photosResourse = new Http(ALBUM_PHOTOS_URL);
...
function fetchAlbums() {
    albumsResourse.list().then(setAlbums)
                            .then(renderAlbums)
...
function getPhotos(id) {
    photosResourse.getOne(id).then(setPhotos).then(renderPhotos);
};
