class TodoListView {
    ...
    //generateItemHtml(item) хороший статический метод, который можно вызвать через имя класса
    generateItemHtml(item) {
        return `<li class="${item.completed ? 'done' : ''} todo" data-id="${item.id}">
                    ${item.title}
    ...
    //getElementId(el) хороший статический метод
    getElementId(el) {
